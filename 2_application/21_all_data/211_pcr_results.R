...
pdf(file.path("plots", "Figure04.pdf"), 10.5, 6)
...
half_probs <- nl_best$prob[,c(8, 3, 7, 5)]

dt <- as.POSIXct(time, format = "%Y-%m-%dT%T")

probs <- data.frame(Prediction = c(lin_probs, half_probs), 
...
# 296 x 3 x 4 
lin_mn <- c(linear_best$mn[,,c(10, 1, 5, 7)])
half_mn <- c(nl_best$mn[,,c(8, 3, 7, 5)])

mns <- data.frame(Prediction = c(lin_mn, half_mn), 
...
pdf(file.path("plots", "Figure05.pdf"), 17.7, 8.6)
...
pdf(file.path("plots", "Figure12.pdf"), 17.7, 8.6)
...
pdf(file.path("plots", "Figure13.pdf"), 17.7, 8.6)
...
pdf(file.path("plots", "Figure14.pdf"), 17.7, 8.6)
...
set.seed(0)
PC3_ice_X_nl <- make_hidden_nodes(PC3_ice_X, n.h, 0.5)
set.seed(0)
PC4_ice_X_nl <- make_hidden_nodes(PC4_ice_X, n.h, 0.5)
...
pdf(file.path("plots", "Figure08.pdf"), 13.2, 11.7)
PC1_plot
graphics.off()
...
pico1_diam <- PC1_ice_list[[10]] + 
  scale_x_continuous(breaks = seq(-8, 8, by = 4), 
                     labels = xaxis_labs) + 
  labs(title = expression(italic("PicoEukaryote") * " " * "Cluster 1"), y = "Diameter", x = "") + 
  theme(plot.title = element_text(size = 20), 
...
leg <- get_legend(pico1_diam) %>% 
  as_ggplot()

pico1_diam <- pico1_diam + 
  theme(legend.position = "none")
...
pdf(file.path("plots", "Figure06.pdf"), 18.3, 5.6)
grid.arrange(pro_diam, pico1_diam, pico2_prob, leg, x_lab, nrow = 2, 
...
plot_ice <- function(PC_lin_ice, PC_nl_ice, PC_num, PC_seq, return_plot_list = FALSE) {
  ...
  PC_lin_diam$PC1 <- PC_nl_diam$PC1 <- factor(other_ice_X[,1], levels = c(-4, 1, 6), 
                                          labels = c("-4", "1", "6"))
  ...
  PC_lin_chl$PC1 <- PC_nl_chl$PC1 <- factor(other_ice_X[,1], levels = c(-4, 1, 6), 
                                          labels = c("-4", "1", "6"))
  ...
  PC_lin_pe$PC1 <- PC_nl_pe$PC1 <- factor(other_ice_X[,1], levels = c(-4, 1, 6), 
                                          labels = c("-4", "1", "6"))
  ...
  PC_pred_melt$PC1 <- factor(PC_pred_melt$PC1, 
                               levels = c(-4, 1, 6), 
                               labels = c("4 (39.6° N)", "-1 (33.2° N)", "-6 (28.5° N)"))
...
# Appendix Figure 9
pdf(file.path("plots", "Figure09.pdf"), 12.5, 11.4)
PC2_plot
graphics.off()
...
pico1_diam <- PC2_plist[[10]] + 
  theme(legend.position = "none") + 
  labs(x = "", title = expression(italic("PicoEukaryote") * " " * "Cluster 1"))
...
POI_colors <- c("black", "#F8766D", "#00BA38", "#619CFF", "black", "black")
...
pdf(file.path("plots", "Figure07.pdf"), 17.5, 10.6)
grid.arrange(pro_prob, syn_pe, pico1_diam, PC2_leg, PC2_xlab, 
