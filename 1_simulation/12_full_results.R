...
simnames <- unite(sims, simnames, sep = "-")$simnames
cvres_files <- file.path(simnames, paste0(simnames, "_summary.RDS"))
...
# load training simulation data and return
get_simdata <- function(
    fname, 
    training_data = TRUE, 
    simdata_dir = file.path("1_simulation", "simdata")
) {
    # extract the name of the folder (name of the experiment)
    simdata_fname <- fname %>% 
        strsplit("/") %>% 
        unlist()
    simdata_fname <- simdata_fname[1]

    # split the string into a vector of settings
    simdata_fname <- simdata_fname %>% 
        strsplit("-") %>% 
        unlist()
    
    # Paste the settings into the simdata file name
    simdata_fname <- paste0(
        "simdata", 
        "_imean_", 
        simdata_fname[1],
        "_iprob_", 
        ...
        simdata_fname[3], 
        "_dataSeed_",
        as.integer(training_data), 
        ".RDS"
    )

    # Get full path
    simdata_fname <- file.path(simdata_dir, simdata_fname)

    # Load and return
    simdata <- readRDS(simdata_fname)
    return(simdata)
}
...
results_list_file <- file.path("1_simulation", "results", "00_results_list.RDS")
...
if(!file.exists(results_list_file)) {
    ...
    results_list <- lapply(cvres_files, function(cv_file) {
        # Load CV results
        cv <- readRDS(file.path("1_simulation", "results", cv_file))
        is_data <- get_simdata(cv_file)
        oos_data <- get_simdata(cv_file, training_data = FALSE)
        ...
        model_fit <- if(params[4] == "NA") "Linear" else "Nonlinear"
    ...
    saveRDS(results_list, file = results_list_file)
} else {
    # Load the results list if this step has already been done
    results_list <- readRDS(results_list_file)
}

results_file <- file.path("1_simulation", "results", "00_results.RDS")
...
new_results <- results %>%
    group_split(scenario) %>%  # Split across each data scenario (linear data, interaction mean, etc.)
...
pdf(file.path("plots", "Figure_03.pdf"), 16, 7.9)
...
is_data <- results_list[[93]]$is_data

true_1_upper <- is_data$mean_spec[,1] + 1.96 * 0.2
true_1_lower <- is_data$mean_spec[,1] - 1.96 * 0.2

true_2_upper <- is_data$pico_mu + 1.96 * 0.2
true_2_lower <- is_data$pico_mu - 1.96 * 0.2
...
p11 <- plot_sim_means(is_data, results_list[[93]]$cvobj, plot_model = TRUE) + 
...
p12 <- plot_sim_means(is_data, results_list[[123]]$cvobj, plot_model = TRUE) + 
    geom_line(data = ci_df, 
             mapping = aes(x = x, y = y,
                           group = interaction(Cluster, type), 
                           linetype = clust, 
