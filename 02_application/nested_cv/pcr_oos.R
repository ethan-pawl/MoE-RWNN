...
load(file.path("~/00_Cyto/data/in_sample_folds.RData"))
...
if(cv_step == "maxres") {
  ...
  destin <- file.path("results", paste0(proj_name, "_", job$model, "_", job$outer_fold))
  if(!dir.exists(destin)) {
    dir.create(destin, recursive = TRUE)
  }

  maxres_file <- paste0(proj_name, "_maxres_", job$model, "_", job$outer_fold, 
                        ".Rdata")

  if(job$model == "half") {
    set.seed(0)
    X <- X_hidden(X_pc, p, n.h, 0.5)
  } else {
    X <- X_pc
  }
  ...
  for(j in -9:0 + 10 * arraynum) { # TODO: check this
    job <- job_grid[j,]
    ...
    seedfile <- file.path("seedtabs", paste0(proj_name, "_seedtab_", job$model, "_", job$outer_fold, ".csv"))
