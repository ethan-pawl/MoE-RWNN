...
n.h <- 70
a <- 0.5
X_nl <- flowmix::make_hidden_nodes(X_pc, n.h, a)
X <- X_nl 
save(X, file = file.path("data", "X_nl.Rdata"))

# We use the original PCs for modeling, but we plot -PC1 (a sign flip)
...
pc_df <- data.frame(Time = rep(dt, 9), 
           Data = as.vector(X_pc), 
...
which_pc <- c("PC3", "PC3", "PC4", "PC4", "PC1", 
              "PC1", "PC2", "PC1", "PC1", "PC1", 
              "PC1", "PC1", "PC1", "PC1", "PC2", 
              "PC2", "PC2", "PC3", "PC3", "PC2", 
              "PC1", "PC1", "PC2", "PC2", "PC2", 
...
focus <- c(TRUE, TRUE, TRUE, TRUE, TRUE, 
...
cov_df <- data.frame(Time = rep(dt, ncol(X_cont)), 
                     Data = as.vector(X_cont), 
                     Variable = rep(short_names, 
                                    each = 296), 
                     Modeled = TRUE,
                     isPC = FALSE)
...
big_df$Variable <- factor(big_df$Variable, levels = c(paste0("PC", 1:9), 
                                                      short_names[c(37, 1:36)], 
                                                      "Latitude"))

# Assign colors, line widths, and colors to variables, PCs, and latitude
brewer_cols <- brewer.pal(12, "Set3")

named_colors <- c("black", 
                  brewer_cols[4], 
                  brewer_cols[5], 
                  brewer_cols[10], 
                  brewer_cols[6], 
...
names(named_colors) <- names(named_widths) <- names(named_types) <- c("PC1", 
                                                                      "SSS", 
                                                                      "SST", 
                                                                      "NO3", 
                                                                      "PO4", 
...
PC1_plot <- ggplot(filter(big_df, PC == "PC1", Focus)) + 
    geom_line(aes(Time, Data, color = Variable, linetype = Variable, 
                  linewidth = Variable)) + 
    ...
    theme(plot.title = element_text(size = 20),
          ...
          legend.title = element_text(size = 18, hjust = 0.5, margin = margin(l = 10, b = 10)),   # Title size
          legend.text = element_text(size = 16, margin = margin(l = 5, r = 10)),    # Item text size
...
names(colors_34) <- names(widths_34) <- names(alphas_34) <- c("PC3", 
                                                              "PC4", 
                                                              "PAR", 
                                                              "p1", 
                                                              "p2", 
...
PC34_plot <- ggplot(filter(big_df, PC %in% c("PC3", "PC4"), Focus)) + 
    ...
    theme(plot.title = element_text(size = 20),
          legend.position = "bottom",
          legend.title = element_text(size = 18, hjust = 0.5, margin = margin(l = 5, b = 10)),   # Title size
          legend.text = element_text(size = 16, margin = margin(l = 5, r = 10)),    # Item text size
          legend.key.size = unit(2, "lines"), 
          text = element_text(size = 18), 
          axis.title = element_text(size = 16), 
          ...
          axis.text.x = element_text(angle = 35, hjust = 1), 
          legend.box.margin = margin(r = 10)) + 
    labs(title = "Principal Components 3 & 4 and Covariates over Time", x = "", y = "") + 
    guides(color = guide_legend(title.position = "top", 
                                override.aes = list(linewidth = c(1.5, 
                                                                  1.5, 
                                                                  1, 
                                                                  1, 
                                                                  1, 
...
# Extract legends
leg1 <- get_legend(PC1_plot) %>% as_ggplot()
PC1_plot <- PC1_plot + theme(legend.position = "none")

leg34 <- get_legend(PC34_plot) %>% as_ggplot()
PC34_plot <- PC34_plot + theme(legend.position = "none")
...
names(colors_2) <- names(widths_2) <- c("PC2", 
                                        "Fe", 
                                        "SLA", 
                                        "Wind Speed")

PC2_plot <- ggplot(filter(big_df, PC == "PC2", Variable %in% c("PC2", "Fe", "SLA", "Wind Speed"))) + 
    geom_line(aes(Time, Data, color = Variable, linewidth = Variable)) + 
    ...
    theme(plot.title = element_text(size = 20),
          legend.position = "bottom",
          legend.title = element_text(size = 18, hjust = 0.5, margin = margin(b = 10)),   # Title size
          legend.text = element_text(size = 16, margin = margin(r = 10)),    # Item text size
          legend.key.size = unit(2, "lines"), 
          text = element_text(size = 18), 
          axis.title = element_text(size = 16), 
          axis.text = element_text(size = 14), 
...
# Appendix Figure 11
time_dictionary <- data.frame(date = rownames(X_pc) %>% as_datetime(),
                             time = 1:296)
all_pc_plot <-
  ...
  select(-time) %>%
  pivot_longer(-date)%>%
  ggplot() + geom_line(aes(x = date, y = value, group = name), linewidth = 1.1) +
  facet_wrap(~name) +
  ylab("") +
  xlab("") +
