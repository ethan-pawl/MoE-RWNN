...
per_cluster_res <- readRDS(file.path("5_competitors", "50_sim", "501_gam", "bam_res.RDS"))
...
if(plot_animation) {
  ...
  plots_path <- file.path(
    "5_competitors", "50_sim", "501_gam", "plots"
  )
...
if(plot_mean_responses) {
  ...
  ggplot(full_df_long |> filter(!(`Data Type` == "Relative Abundance" & Cluster %in% paste0("Cluster 2 ", c("Truth", "Estimate"))))) + 
...
rmse # [1] 0.3416356 0.3756268
sum(rmse) # [1] 0.7172623
...
cov_err
# [1] 0.02298208 0.03480707

# TODO: try multivariate algorithm